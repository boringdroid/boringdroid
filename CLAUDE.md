...
- **Overlay panels must sit above the 64dp taskbar, not overlap it.** The taskbar (`taskbar_window_height = 64dp`) is the bottom anchor for every other plugin surface. Any panel anchored with `Gravity.BOTTOM` needs a `margin_bottom` ≥ 64dp plus a ~20dp breathing gap — panels use 88dp today. Any panel anchored with `Gravity.TOP` must subtract `R.dimen.taskbar_window_height` from its computed `y` so the bottom edge clears the taskbar. Fullscreen overlays (e.g. Overview) must not use `MATCH_PARENT` height — shrink the window height by the taskbar so its window chrome stays interactable underneath.
